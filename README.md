...
netdump 是一个用于验证和探索 Linux 环境下高性能数据采集与进程间数据流设计的实验性项目。

本项目的核心目标是围绕特定技术问题进行设计取舍与实现验证，而非提供一个功能完整、可直接用于生产环境的抓包工具。
...
+ 基于 mmap / 共享内存的零拷贝数据传递
+ 数据采集与数据解析的进程解耦
+ 高吞吐场景下的数据路径开销控制
+ 数据处理过程中的延时与可观测性

为保持实现的可控性与可分析性，以下内容不在本项目的设计目标范围内：

- 完整的协议解析或深度包检测功能
- 长期存储、索引或分析历史数据
- 面向生产环境的容错、权限控制与安全加固
- 与 tcpdump、Wireshark 等成熟工具的功能对标
